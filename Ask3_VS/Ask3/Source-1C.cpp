...
std::pair<GLfloat, GLfloat> createRandomCoordinates() {
    ...
    double min_x_decimal, min_y_decimal;
    bool collision;
    ...
    } while (collision || min_x <= -5.0f || min_y <= -5.0f || max_x >= 5.0f || max_y >= 5.0f);
...
int main(void) {
    ...
    if (background_sound) {
        background_sound->setVolume(0.6f);
        background_sound->drop();
        background_sound = 0;
    }
